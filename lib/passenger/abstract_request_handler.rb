  ...
  def reap_dead_workers
    if @workers.list.length > 0
      ...
      @workers.list.each do |worker|
        worker[:started_on] = Time.now unless worker[:started_on]
        if mark - worker[:started_on] > 60
          STDERR.puts("#{Time.now} Thread #{worker.inspect} is too old, killing.")
          STDERR.flush
          worker.raise(TimeoutError.new("Passenger has timed out thre thread"))
        end
      end
    end
